...
//			int cnt = 
	...
	public void insertRent(RentalVO rent) {
		...
		//수정(유지보수의 입장에선 좋은가? 안좋은가??): 일단 table create 시에 위의 쿼리문을 삽입해서 기본적으로 일자가 늘어나게 할 수 있음
...
//		}
//		if (cnt != 0) {
//			updateIsRented(rent.getIsbn(), 1);
//		}
//	}
//
//	public void updateIsRented(String isbn, int a) {
//		StringBuilder sql = new StringBuilder();
//		sql.append("UPDATE T_Book SET is_rented = ? WHERE BookISBN = ?");
//
//		try (Connection conn = new ConnectionFactory().getConnection();
...
//		StringBuilder sql = new StringBuilder();
//		sql.append("INSERT INTO T_Rent(RentID, UserID, BookISBN, Title, RentalDate, ReturnDate) ");
//		sql.append("VALUES(nextval('seq_RentID'), ?, ?, ?, CURRENT_DATE, CURRENT_DATE + INTERVAL '14' DAY)");
//
//		int cnt = 0;
//		try (Connection conn = new ConnectionFactory().getConnection();
//				PreparedStatement pstmt = conn.prepareStatement(sql.toString());) {
...
//
//		if (cnt != 0) {
//			updateIsRented(rent.getIsbn(), 0);
//		}
//	}
//
//	public List<RentalVO> selectAll() {
//
//		List<RentalVO> rentList = new ArrayList<>();
//
//		StringBuilder sql = new StringBuilder();
//		sql.append("SELECT RentID, UserID, BookISBN, Title, RentalDate, ReturnDate ");
//		sql.append("FROM T_Rent ");
...
//
//				RentalVO rent = new RentalVO(rentID, userID, bookISBN, title, rentalDate, returnDate);
//				rentList.add(rent);
//			}
//
//		} catch (Exception e) {
...
//	}
//
//	public List<RentalVO> selectId(String id) {
//
//		List<RentalVO> rentList = new ArrayList<>();
//
//		StringBuilder sql = new StringBuilder();
//		sql.append("SELECT * FROM T_Rent WHERE UserID = ? ");
//
//		try (
//				Connection conn = new ConnectionFactory().getConnection();
//				PreparedStatement pstmt = conn.prepareStatement(sql.toString());) {
