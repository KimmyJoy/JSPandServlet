...
<%-- 버튼 단 --%>
		...
		<c:choose>
			<c:when test="${ empty bookList and empty allBooks }">
				...
				<img alt="검색결과 없음" src="./source/src-img/gr-magnifying-glass.png"
					width="50px">
				<h6>'${ param.searchKeyword }'에 대한 검색 결과가 없음!</h6>
			</c:when>
			<c:when test="${ not empty allBooks and empty bookList }">
	...
	<!-- 도서 대출 모달 -->
	<div class="modal fade" id="rentBook" tabindex="-1" role="dialog"
		aria-labelledby="rentBookLabel" aria-hidden="true">
		<jsp:include page="../login.jsp"></jsp:include>
	</div>
